...
bot = commands.Bot(command_prefix="!", case_insensitive=True, help_command=commands.DefaultHelpCommand(),
                      intents=intents)
...
async def load_extensions():
    for file in listdir('cogs_mlr_scout/'):
        if file.endswith('.py'):
            await bot.load_extension(f'cogs_mlr_scout.{file[:-3]}')


async def main():
    async with bot:
        await load_extensions()
        await bot.start(os.getenv("MLR_SCOUT_DISCORD_TOKEN"))
